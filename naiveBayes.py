...
class NaiveBayesClassifier(classificationMethod.ClassificationMethod):
    ...
    def findHighOddsFeatures(self, label1, label2):
        ...
        #     featuresOdds.append((self.conditionalProb[i, label1]/self.conditionalProb[i, label2], i))
        #     featuresOdds.sort()
        #     featuresOdds = [i for val, i in featuresOdds[-100:]]

        # return featuresOdds

        for i in self.features:
            odds_ratio = self.conditionalProb[i, label1]/self.conditionalProb[i, label2]
            featuresOdds.append((odds_ratio, i))

        featuresOdds.sort(key=lambda x: x[0], reverse=True)

        top_features = [i for _, i in featuresOdds[:100]]
        return top_features
