...
def enhancedFeatureExtractorDigit(datum):
    ...
    percentTop = float(topCentre) / nonzero
    for n in range(5):
        features[-(n + 1) * 10] = percentTop > 0.35 and 1.0 or 0.0
        
    percentRight = float(right) / nonzero
...
def count_contiguous_regions(datum):
    """
    Count the number of contiguous white regions in the digit.
    Uses connected component labeling algorithm.
    """
    def dfs(x, y, label):
        """
        Depth-First Search (DFS) to label connected white pixels.
        """
        if (x, y) not in visited and datum.getPixel(x, y) == 1:
            visited.add((x, y))
            labels[x][y] = label
    ...
    visited = set()
    num_regions = 0
    labels = [[0 for _ in range(DIGIT_DATUM_HEIGHT)] for _ in range(DIGIT_DATUM_WIDTH)]
